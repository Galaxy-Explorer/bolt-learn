...
func (n *node) write(p *page) {
    ...
    for i, item := range n.inodes {
        ...
        if n.isLeaf {
            ...
            // 为啥是b[0]呢，因为此时指针的位置已经是n.pageElementSize()*len(n.inodes)这个位置了，再往后就是实际的k v值了
            // 所以才是uintptr(unsafe.Pointer(&b[0])) - uintptr(unsafe.Pointer(elem)) 这样才能得到每个elem的位移
